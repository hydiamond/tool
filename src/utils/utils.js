...
export const findLatestBrowserVersionDirectory = (browserPath) => {
  ...
  const directories = folderContents.filter((file) =>
    statSync(join(browserPath, file)).isDirectory(),
  );

  const { folderName, version } = directories.reduce(
    (newest, currentFolderName) => {
